...
var (
	...
	ErrWireExecution        = errors.New("mint engine failed to resolve dependency graph")
...
// MintContainer holds all configured dependencies.
type MintContainer struct {
	{{range .Providers}}
	{{.StructName}} {{if .IsPointer}}*{{end}}{{.PackagePrefix}}{{.StructName}}
	{{end}}
}

// InitializeContainer is generated by mint and resolved by Google Wire.
func InitializeContainer() (*MintContainer, error) {
	wire.Build(
...
func (g *WireGenerator) Generate(outDir string, components []*engine.ComponentMetadata) error {
	...
	tempFilePath := filepath.Join(absOutDir, "mint_injector.go")
	...
	finalMintFile := filepath.Join(absOutDir, "mint_container.go")

	log.Debug("Renaming generated file", "from", generatedWireFile, "to", finalMintFile)
	if err := os.Rename(generatedWireFile, finalMintFile); err != nil {
		chainErr := fmt.Errorf("%w: %w", ErrRenameGeneratedFile, err)
		return errs.Wrap(chainErr, "from %s to %s", generatedWireFile, finalMintFile)
	}
